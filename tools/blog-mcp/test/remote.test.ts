...
describe('remote tools against a gh shim (no real GitHub involved)', () => {
  ...
  it('blog_list_prs sorts by most-recently-updated regardless of gh\'s own return order, and calls the exact argv', async () => {
    // Only two fixtures: the shim (unlike real `gh`) doesn't honor --limit itself,
    // so asserting a `limit: 2` call must not depend on that truncation happening.
    ...
    try {
      ...
      expect(data.prs.map((p) => p.number)).toEqual([12, 10]);
